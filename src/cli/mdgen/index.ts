...
const INSERT_END = ["<!-- END lex generated TOC please keep comment here to allow auto update -->"];
...
export async function process(outFilePath: string, lexicons: LexiconDoc[]) {
	...
	fileLines.splice(startIndex, endIndex - startIndex + 1, [INSERT_START, await genMdLines(lexicons), INSERT_END]);
...
async function genMdLines(lexicons: LexiconDoc[]): Promise<StringTree> {
	...
	for (const lexicon of lexicons) {
		...
		doc.push([`---`, ``, `## ${lexicon.id}`, "", desc, "```json", JSON.stringify(lexicon, null, 2), "```"]);
	}
	return doc;
}
