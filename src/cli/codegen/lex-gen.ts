...
export function genUserType(file: SourceFile, imports: Set<string>, lexicons: Lexicons, lexUri: string) {
...
function genObject(
	...
	if (typeProperty) {
		...
		iface.addProperty({
			...
			type:
				...
				hash === "main" ? JSON.stringify(`${baseNsid}`) : JSON.stringify(`${baseNsid}#${hash}`),
		});
	}
	...
	if (def.properties) {
		for (const propKey in def.properties) {
			...
			const req =
				def.required?.includes(propKey) || (defaultsArePresent && "default" in propDef && propDef.default !== undefined);
...
export function genArray(file: SourceFile, imports: Set<string>, lexUri: string, def: LexArray) {
...
export function genPrimitiveOrBlob(file: SourceFile, lexUri: string, def: LexPrimitive | LexBlob | LexIpldType) {
	genComment(
		file.addTypeAlias({
			name: toTitleCase(getHash(lexUri)),
...
export function genXrpcParams(file: SourceFile, lexicons: Lexicons, lexUri: string, defaultsArePresent = true) {
	const def = lexicons.getDefOrThrow(lexUri, ["query", "subscription", "procedure"]);
...
export function genRecord(file: SourceFile, imports: Set<string>, lexicons: Lexicons, lexUri: string) {
...
function refToUnionType(ref: string, lexUri: string, imports: Set<string>): string {
	const baseNsid = stripScheme(stripHash(lexUri));
	return `$Typed<${refToType(ref, baseNsid, imports)}>`;
}

function refToType(ref: string, baseNsid: string, imports: Set<string>): string {
...
export function primitiveOrBlobToType(def: LexBlob | LexPrimitive | LexIpldType): string {
	switch (def.type) {
		case "blob":
			return "BlobRef";
		case "bytes":
	...
	switch (def.type) {
		case "string":
			if (def.knownValues?.length) {
				return `${def.knownValues.map((v) => JSON.stringify(v)).join(" | ")} | (string & {})`;
			} else if (def.enum) {
...
function makeType(_types: string | string[], opts?: { array?: boolean; nullable?: boolean }) {
