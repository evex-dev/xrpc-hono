...
export async function genClientApi(lexiconDocs: LexiconDoc[]): Promise<GeneratedAPI> {
...
const indexTs = (project: Project, lexiconDocs: LexiconDoc[], nsidTree: DefTreeNode[], nsidTokens: Record<string, string[]>) =>
	gen(project, "/index.ts", async (file) => {
		...
		file.addImportDeclaration({ moduleSpecifier: "./lexicons.js" }).addNamedImports([{ name: "schemas" }]);
		...
		file.addImportDeclaration({ moduleSpecifier: `./util.js` }).addNamedImports([
		...
		for (const lexicon of lexiconDocs) {
			...
			file.addImportDeclaration({ moduleSpecifier }).setNamespaceImport(toTitleCase(lexicon.id));
			file.addExportDeclaration({ moduleSpecifier }).setNamespaceExport(toTitleCase(lexicon.id));
		}
		...
		clientCls.addConstructor({
			parameters: [{ name: "options", type: "FetchHandler | FetchHandlerOptions" }],
			statements: ["super(options, schemas)", ...nsidTree.map((ns) => `this.${ns.propName} = new ${ns.className}(this)`)],
		});
...
function genNamespaceCls(file: SourceFile, ns: DefTreeNode) {
	...
	for (const userType of ns.userTypes) {
		...
		method.setBodyText(
			[
				`return this._client`,
				isGetReq ? `.call('${userType.nsid}', params, undefined, opts)` : `.call('${userType.nsid}', opts?.qp, data, opts)`,
...
function genRecordCls(file: SourceFile, nsid: string, lexRecord: LexRecord) {
	...
	}
	{
		...
		method.setBodyText(
			[`const res = await this._client.call('${ATP_METHODS.get}', { collection: '${nsid}', ...params })`, `return res.data`].join(
				"\n",
			),
		);
	}
	{
		...
		const maybeRkeyPart = lexRecord.key?.startsWith("literal:") ? `rkey: '${lexRecord.key.replace("literal:", "")}', ` : "";
		method.setBodyText(
			[
	...
	}
	{
		...
		method.setBodyText(
			[`await this._client.call('${ATP_METHODS.delete}', undefined, { collection: '${nsid}', ...params }, { headers })`].join(
				"\n",
			),
		);
	}
...
const lexiconTs = (project, lexicons: Lexicons, lexiconDoc: LexiconDoc) =>
	gen(project, `/types/${lexiconDoc.id.split(".").join("/")}.ts`, async (file) => {
		...
		if (main?.type === "query" || main?.type === "subscription" || main?.type === "procedure") {
			//= import {HeadersMap, XRPCError} from '@atproto/xrpc'
			const xrpcImport = file.addImportDeclaration({
				moduleSpecifier: "@atproto/xrpc",
			});
			xrpcImport.addNamedImports([{ name: "HeadersMap" }, { name: "XRPCError" }]);
		}
...
function genClientXrpcCommon(file: SourceFile, lexicons: Lexicons, lexUri: string) {
	...
	for (const error of def.errors || []) {
		...
		errCls.addConstructor({
			...
			statements: ["super(src.status, src.error, src.message, src.headers, { cause: src })"],
		});
